...
filename:
    ...
    special_script_version_neuro_wm.py
'''
...
# python special_script_version_neuro_wm.py # запуск скрипта обученной сети
...
cover_img = input("Введите путь и имя изображения-контейнера-c-wm с расширением: ")
cover_test = Image.open(f'{cover_img}').convert('RGB')
w_cover, h_cover = cover_test.size
...
test3 = net.decode(cover_test.unsqueeze(0)) # конвертируем картинки в тензоры для подачи в нейросеть-decoder
...
img3.resize((w_logo, h_logo)).save(f"img/Extract-WM.jpg")